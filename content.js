...
async function scroll_page() {
    ...
    while(timeout > Date.now()) {
        ...
        if(i % 120 === 0) {
            ...
            write_log(`Waiting ${seconds} seconds...`);
        }
        i += 1;
    }
    ...
    while (is_running) {
        ...
        if (testing_retry) {
            if (loops_since_retry > 1) {
                // wait and refresh
                write_log('No new posts after retrying. Waiting 5 minutes and refreshing before checking again.');
                let new_url;
        ...
        if (last_seen_post_id === previous_last_post_id) {
            write_log(`No new posts detected in this loop (${loops_since_retry + 1})`);
            if (!testing_retry) {
                testing_retry = true;
            }
        }
        await wait(1000); // Small pause between iterations to avoid hammering X.com
    }

    write_log('Stop requested, ending loop.');
    browser.runtime.sendMessage({done: true}); // Signal that the process is done
...
async function scroll_to_bottom() {
    ...
    const scrollspeed = get_pixel_speed(3500);
